...
Chainlit을 기반으로 하여, 사용자의 질문에 대해 **생성형 AI의 답변**과 **데이터베이스 내 가장 유사한 질문-답변(Reference)**을 동시에 제공합니다.

다른 개발자들이 이 코드를 기반으로 자신의 데이터(사규, 매뉴얼, 교육 자료 등)를 넣어 커스터마이징할 수 있도록 설계되었습니다.
...
* **자동 벡터 저장소 구축**: 엑셀(XLSX)이나 PDF 파일을 넣고 스크립트만 실행하면 자동으로 FAISS 벡터 DB를 구축합니다.
* **사용자 만족도 기록**: 사용자의 만족도 피드백을 오픈소스 DB인 MariaDB에 저장하여 추후 모델 개선에 활용합니다.

## 기술 스택

* **UI/UX**: [Chainlit](https://docs.chainlit.io)
* **LLM**: Google Gemini Pro (Cloud), Ollama (Local)
* **RAG/벡터 저장소**: FAISS (CPU based), LangChain
* **Database**: MariaDB / MySQL (로그 및 피드백 저장)
* **NLP**: Kiwi (한국어 형태소 분석), HuggingFace Embeddings
...
* 운영체제: 리눅스
* Python: 3.13 이상
* 오픈소스 LLM 사용 시 [Ollama](https://ollama.com/) 설치 및 모델 다운로드 (`gemma3:12b-it-qat` 권장)
* 데이터베이스: MariaDB
...
이 챗봇을 **여러분의 목적(법률, 의료, 사내 규정 등)**에 맞게 바꾸려면 데이터를 교체하세요.

    data/ 폴더: 질문-답변 쌍이 담긴 엑셀 파일(df_qna_*.xlsx)을 넣습니다. (컬럼: 질문, 답변)

    pdfs/ 폴더: 참고할 PDF 문서를 넣습니다.
